...
class UserController extends FleetbaseController
{
    ...
    /**
     * Compare ETags for cache validation, handling weak ETags and compression suffixes.
     *
     * @param string $clientETag ETag from client's If-None-Match header
     * @param string $serverETag ETag generated by server
     *
     * @return bool
     */
    private function etagsMatch(string $clientETag, string $serverETag): bool
    {
        // Normalize both ETags by removing weak prefix and compression suffixes
        $normalizeETag = function ($etag) {
            // Remove W/ prefix for weak ETags
            $etag = preg_replace('/^W\//', '', $etag);
            // Remove quotes
            $etag = trim($etag, '"');
            // Remove compression suffixes like -gzip, -br, -zstd, etc.
            $etag = preg_replace('/-(?:gzip|br|zstd|deflate)$/', '', $etag);
